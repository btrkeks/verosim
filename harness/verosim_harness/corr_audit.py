...
Usage:
  python -m verosim_harness.corr_audit --oracle oracle/dev200_active.jsonl \\
...
def run_pairs(bin_path: Path, pairs: list[tuple[str, str]], data_root: str, mode: str) -> dict[tuple, dict]:
...
def mode_from_oracle_path(path: Path) -> str:
    stem = path.stem
    for mode in ("experimental", "active"):
        if stem.endswith(f"_{mode}"):
            return mode
    raise ValueError(
        f"cannot infer metric mode from {path}; expected a name ending in "
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--mode",
        choices=("active", "experimental"),
        help="metric mode for the C++ run; inferred from --oracle filename by default",
    )
    ap.add_argument(
        "--explained",
        type=Path,
    ...
    mode_label = args.mode or mode_from_oracle_path(args.oracle)
    cpp = run_pairs(args.bin, pairs, str(args.data_root), mode_label)
    ...
    if args.report:
        ...
        lines = [
            f"# {corpus_label} {mode_label} correlation audit",
            "",
            "C++ `verosim` OMR-NED vs the cached Python oracle "
            f"(`{args.oracle}`, mode {mode_label}). Generated by "
